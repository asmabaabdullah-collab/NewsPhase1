...
# AI Analysis
# =========================
def call_llm_for_analysis(article_title, article_text):
...
# Google News Search
# =========================
def search_related_sources(query, limit=8):
...
if run:
    ...
    if not article_text.strip():
        st.error("No article text found.")
        st.stop()
    ...
    tab1, tab2, tab3 = st.tabs(
        ["Summary", "Related Sources Found", "Export"]
    )
    ...
    with tab1:
        ...
        st.write(
            analysis.get("main_event", "Not available")
        )
